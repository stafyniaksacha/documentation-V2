...
When using `websocket` or `socket.io` protocol, the client will start receiving [notifications]({{ site_base_path }}api/2/notifications) right away.  
...
* <= Kuzzle forwards the corresponding [notifications]({{ site_base_path }}api/2/notifications/) on that channel
